    ...
    operator fun plus(other: DomainElementInterceptor): DomainElementInterceptor {
        return object : DomainElementInterceptor {
            ...
            override fun transform(element: DomainElement?): DomainElement? {
                val first: DomainElement? =
                    if (this.wantIntercept(element)) this@DomainElementInterceptor.transform(element) else element
                val second: DomainElement? =
                    if (other.wantIntercept(element)) other.transform(first) else first
                return second
            }
